...
bool Renderer::init_fallback_texture()
{
    ...
    tex_info.width = surface->w;
    tex_info.height = surface->h;
    tex_info.layer_count_or_depth = 1;
    tex_info.num_levels = 1;
    tex_info.format = SDL_GPU_TEXTUREFORMAT_R8G8B8A8_UNORM;
    tex_info.usage = SDL_GPU_TEXTUREUSAGE_SAMPLER;

    fallback_texture = SDL_CreateGPUTexture(device, &tex_info);
    ...
    texture_region.texture = fallback_texture;
    texture_region.w = tex_info.width;
    texture_region.h = tex_info.height;
    texture_region.d = 1;
